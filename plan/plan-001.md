...
4. **spec.md's Buyer-default path references "the Buyer's cash/collateral posted for the rehypothecated position" — but no such posted-collateral mechanism is defined anywhere in spec.md.** There is no on-chain account, in any flow or in Token design, representing margin or collateral the Buyer has posted. Rather than invent a new margin-posting mechanism unsupported by spec.md, this plan resolves the Buyer-default claim as a **status-only transition**: the instruction flips the trade-state's status to `Buyer-defaulted` and emits an event, moving no funds — consistent with spec.md's own statement that only the on-chain *trigger* is in scope and liquidation/enforcement is not. This resolution stands as correct for this POC.
   ...
   **What this deliberately defers, named explicitly rather than left as an unnoticed gap — corrected after reviewing the actual 1996 MRA text (see spec-001.md's Areas of concern, MRA mechanics reviewed and deliberately not modeled, item 5):** a real MRA (¶12's Single Agreement language plus ¶11(a)'s Events of Default) treats a default on any one Transaction as an immediate Event of Default across *every* other outstanding Transaction between the same two parties, with the non-defaulting party entitled to net exposure across the whole book — a cross-default/cross-acceleration mechanism, considerably blunter than the transaction-scoped "mini close-out" an earlier draft of this note attributed to MRA (that framing may be specific to GMRA or a later amendment; it isn't confirmed present in the base 1996 MRA text this project is built on). This POC's status-only Buyer-default flag implements neither mechanism — it only marks that the underlying failure occurred — and that resolution remains correct regardless of which real-world mechanism is the accurate comparison, since implementing either would require a market-value input for the defaulted securities (netting) or a genuine multi-trade netting/cross-default engine (cross-acceleration), both real, separate work intent-001.md's negotiation-logic exclusion and this POC's single-Transaction scope keep out of reach here.
